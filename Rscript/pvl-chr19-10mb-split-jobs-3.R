...
readRDS("data-to-condor/expr_anchors.rds") -> foo
foo[ , 2, drop = FALSE] -> anchor
colnames(foo)[2] -> anchor_id
# define phenames
phenames <- c(trait_id, anchor_id)
...
pheno <- as.matrix(cbind(locals[ , trait_indic, drop = FALSE], anchor))
